...
it('supports the MCP initialize, list and call lifecycle using the SDK client', async () => {
  ...
  try {
    ...
    const hexbin = await client.callTool({ name: 'visualization_render_svg', arguments: {
      chartId: 'hexbin', x: 'x', y: ['y'], table: { columns: ['x', 'y'], rows: [[0, 0], [0, 0], [5, 10]] },
    } })
    expect(hexbin.isError).not.toBe(true)
    expect(JSON.stringify(hexbin.content)).toContain('共 3 条')
    const horizon = await client.callTool({ name: 'visualization_render_svg', arguments: {
      chartId: 'horizon', x: 'time', y: ['value'], table: { columns: ['time', 'value'], rows: [[0, -3], [1, 3]] },
    } })
    expect(horizon.isError).not.toBe(true)
    expect(JSON.stringify(horizon.content)).toContain('负值向上折叠')
    const packing = await client.callTool({name:'visualization_render_svg',arguments:{
      chartId:'circle-packing',x:'node',parent:'parent',y:['weight'],
      table:{columns:['node','parent','weight'],rows:[['root',null,null],['A','root',4],['B','root',1]]},
    }})
    expect(packing.isError).not.toBe(true)
    expect(JSON.stringify(packing.content)).toContain('父圆表示包含关系')
    const contour=await client.callTool({name:'visualization_render_svg',arguments:{
      chartId:'contour',x:'x',y:['y','height'],table:{columns:['x','y','height'],rows:[[0,0,0],[1,0,2],[0,1,2],[1,1,4]]},
    }})
    ...
    const dashboard = await client.callTool({
      ...
      arguments: {
        ...
        cards: ['bar', 'kpi'].map((chartId) => ({
          ...
          x: 'name',
          y: ['value'],
          table: { columns: ['name', 'value'], rows: [['甲', 3], ['乙', 7]] },
        })),
      },
    })
    ...
    const editable = { title: '可编辑组合', columns: 1, filter: { column: 'name', values: ['甲'] }, cards: [{
      chartId: 'kpi', x: 'name', y: ['value'], table: { columns: ['name', 'value'], rows: [['甲', 3]] },
    }] }
    const exported = await client.callTool({ name: 'visualization_dashboard_export', arguments: editable })
    expect(exported.isError).not.toBe(true)
    const file = JSON.parse((exported.content as { text: string }[])[0]!.text)
    expect(file.filename).toBe('dashboard.zenoffice.json')
    const imported = await client.callTool({ name: 'visualization_dashboard_import', arguments: { content: file.content } })
