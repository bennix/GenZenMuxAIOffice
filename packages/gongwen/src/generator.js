...
const FONT = {
  title: {
    eastAsia: preferredFont(
      ['方正小标宋简体', '方正小标宋_GBK', 'FZXiaoBiaoSong-B05S', 'FZXiaoBiaoSong-B13S'],
      'Songti SC',
    ),
  ...
  subtitle: {
    eastAsia: preferredFont(['仿宋', '仿宋_GB2312', 'STFangsong'], 'STFangsong'),
    ascii: 'Times New Roman',
    cs: 'Times New Roman',
  },
  body: {
    eastAsia: preferredFont(
      ['仿宋', '仿宋_GB2312', 'FangSong', 'FangSong_GB2312', 'STFangsong'],
      'STFangsong',
    ),
    ascii: 'Times New Roman',
    cs: 'Times New Roman',
  ...
  h1: {
    eastAsia: preferredFont(['黑体', 'SimHei', 'Heiti SC', 'STHeiti'], 'Heiti SC'),
    ascii: 'Times New Roman',
    cs: 'Times New Roman',
  },
  h2: {
    eastAsia: preferredFont(['楷体', '楷体_GB2312', 'KaiTi', 'Kaiti SC', 'STKaiti'], 'Kaiti SC'),
    ascii: 'Times New Roman',
    cs: 'Times New Roman',
  },
...
function paragraph(text, opts = {}) {
  ...
  const outline =
    opts.outlineLevel === undefined ? '' : `<w:outlineLvl w:val="${opts.outlineLevel}"/>`
  const pageBreak = opts.pageBreakBefore ? '<w:pageBreakBefore/>' : ''
  const snapToGrid =
    opts.snapToGrid === false ? '<w:snapToGrid w:val="false"/>' : '<w:snapToGrid w:val="true"/>'
...
function upwardHeaderParagraph(docNo, signer) {
  ...
  const signers = String(signer)
    .split(/[、,，;；\s]+/)
    .map((name) => name.trim())
    .filter(Boolean)
  if (signers.length > 2)
    throw new Error(
      "upward --signer supports up to two names; use the authority's joint-signature template for more signers",
    )
  const signerRuns = signers
    .map(
      (name, index) =>
        `${index ? run('　　', { fontPreset: 'h2', size: '32' }) : ''}${run(name, { fontPreset: 'h2', size: '32' })}`,
    )
    .join('')
...
function normalizeDocumentNumber(text, label = '--doc-no') {
  ...
  if (!/^[^〔〕[\]【】]+〔\d{4}〕[1-9]\d*号$/.test(value) || /第/.test(value)) {
    throw new Error(
      `${label} must use 年份全称、六角括号、非虚位顺序号且以“号”结尾，例如“示例发〔2026〕1号”`,
    )
  }
  return value
}
...
function attachmentNote(text) {
  return paragraph(normalizeAttachmentNote(text), {
    align: 'left',
    fontPreset: 'body',
    size: '32',
    before: '560',
...
function estimatedTextWidthDxa(text) {
  return [...String(text ?? '')].reduce(
    (sum, char) => sum + (char.codePointAt(0) <= 0x7f ? 160 : 320),
    0,
  )
}
...
function signatureParagraphs(args) {
  if (args['seal-mode'] === 'signed') {
    if (!args.signer || !args['signer-title'])
      throw new Error('signed seal mode requires --signer and --signer-title')
    const lines = [
      rightWithIndent(`${args['signer-title']}  ${args.signer}`, FIRST_LINE_INDENT * 2),
    ]
...
function colophon(args) {
  ...
  if (args['print-org'] || args['print-date']) {
    ...
    body.push(
      `<w:tr><w:tc><w:tcPr><w:tcW w:w="${CONTENT_W}" w:type="dxa"/><w:tcMar><w:top w:w="0" w:type="dxa"/><w:left w:w="0" w:type="dxa"/><w:bottom w:w="0" w:type="dxa"/><w:right w:w="0" w:type="dxa"/></w:tcMar></w:tcPr>${printRow}</w:tc></w:tr>`,
    )
  }
  ...
  return [
    `<w:tbl>${tablePr}<w:tblGrid><w:gridCol w:w="${CONTENT_W}"/></w:tblGrid>${body.join('')}</w:tbl>`,
  ]
}
...
function tableCell(text, width, header) {
  ...
  const pPr =
    '<w:pPr><w:spacing w:before="0" w:after="0" w:line="560" w:lineRule="exact"/><w:jc w:val="left"/></w:pPr>'
  const r = run(text, header ? { fontPreset: 'h1', size: '32' } : { size: '32' })
...
function stripTrailingMatchingSignature(blocks, args) {
  if (
    args.date &&
    (isMatchingParagraph(blocks.at(-1), args.date) || isChineseDateParagraph(blocks.at(-1)))
  ) {
    blocks.pop()
  }
...
function looksLikeMainSend(text) {
  ...
  return /(各|贵|省|市|县|区|镇|乡|机关|单位|公司|部门|委员会|办公室|厅|局|处|中心|领导|集团|党委|党总支|党支部)/.test(
    compact.slice(0, -1),
  )
}
...
function buildDocument(blocks, args) {
  ...
  const format = ['ordinary', 'formal', 'letter', 'command', 'minutes'].includes(args.format)
    ? args.format
    : 'ordinary'
  ...
  if (
    ...
    sourceBlocks[0].text.trim() === title.trim()
  ) {
    sourceBlocks.shift()
  }
  ...
  if (
    !mainSend &&
    sourceBlocks[0]?.type === 'paragraph' &&
    looksLikeMainSend(sourceBlocks[0].text)
  ) {
  ...
  if (formal) {
    if (letterhead === 'digital' && !args.org)
      throw new Error('formal digital letterhead requires --org')
    const headerFieldCount = [args['copy-no'], args.secret, args.urgent].filter(Boolean).length
    if (args['copy-no'] && !/^\d{1,6}$/.test(String(args['copy-no'])))
      throw new Error('--copy-no must contain one to six Arabic digits')
    if (args['copy-no'])
      body.push(headerFieldParagraph(String(args['copy-no']).padStart(6, '0'), 'body'))
    if (args.secret) body.push(headerFieldParagraph(args.secret))
    if (args.urgent) body.push(headerFieldParagraph(args.urgent))
    if (letterhead === 'digital' && args.org) {
      body.push(
        agencyMarkParagraph(args.org, String(Math.max(0, 1984 - headerFieldCount * 560)), '0'),
      )
      if (args['doc-no']) body.push(...blankGridLines(2))
    }
    if (letterhead === 'preprinted') {
      const reserve = Number(args['letterhead-reserve-mm'] ?? 72)
      if (!Number.isFinite(reserve) || reserve < 37 || reserve > 130)
        throw new Error('--letterhead-reserve-mm must be between 37 and 130')
    ...
    if (upward && args['doc-no'] && args.signer)
      body.push(upwardHeaderParagraph(args['doc-no'], args.signer))
  ...
  if (args['print-date'])
    args['print-date'] = normalizeChineseDate(args['print-date'], '--print-date', true)
  if (title) {
  ...
  if (args.note)
    body.push(
      paragraph(`（${args.note.replace(/^（|）$/g, '')}）`, {
        align: 'left',
        fontPreset: 'body',
        size: '32',
      }),
    )
  const attachmentFiles = Array.isArray(args['attachment-file'])
  ...
  if (format === 'minutes' && args.attendees)
    body.push(minutesPersonParagraph('出席', args.attendees, '560'))
  if (format === 'minutes' && args.absent) body.push(minutesPersonParagraph('请假', args.absent))
  if (format === 'minutes' && args.observers)
    body.push(minutesPersonParagraph('列席', args.observers))
  if (format !== 'letter') body.push(...colophon(args))
  const letterFooter =
    format === 'letter' ? '<w:footerReference w:type="default" r:id="rIdLetterFooter"/>' : ''
  const footerRefs =
    letterFooter ||
  ...
  body.push(
    `<w:sectPr>${footerRefs}<w:pgSz w:w="${PAGE_W}" w:h="${PAGE_H}"/><w:pgMar w:top="${pageMarginTop}" w:right="${MARGIN.right}" w:bottom="${MARGIN.bottom}" w:left="${MARGIN.left}" w:header="720" w:footer="${footerDistance}" w:gutter="0"/><w:docGrid w:type="lines" w:linePitch="560"/></w:sectPr>`,
  )
...
function footerXml(align = 'center') {
  ...
  const sideIndent =
    align === 'right' ? '<w:ind w:right="280"/>' : align === 'left' ? '<w:ind w:left="280"/>' : ''
...
async function writeDocx(documentXml, pageNumberMode, format) {
  const zip = new JSZip()
  const footerTypes =
    format === 'letter'
  ...
  zip.file('word/_rels/document.xml.rels', documentRelsXml(pageNumberMode, format))
  zip.file('word/document.xml', documentXml)
  zip.file('word/styles.xml', stylesXml())
  zip.file('word/fontTable.xml', fontTableXml())
  zip.file('word/settings.xml', settingsXml(pageNumberMode))
  if (format === 'letter') {
    zip.file('word/footerLetter.xml', letterFooterXml())
  } else if (pageNumberMode === 'standard') {
    zip.file('word/footerOdd.xml', footerXml('right'))
    zip.file('word/footerEven.xml', footerXml('left'))
  } else if (pageNumberMode === 'center') {
    zip.file('word/footerCenter.xml', footerXml('center'))
  }
  return zip.generateAsync({ type: 'nodebuffer', compression: 'DEFLATE' })
}

export async function generateGongwen(markdown, options = {}) {
  if (typeof markdown !== 'string' || !markdown.trim()) throw new Error('请输入公文正文。')
  if (markdown.length > 1000000) throw new Error('正文不能超过 100 万字符。')
  const allowed = [
    'format',
    'title',
    'org',
  ...
  for (const key of allowed) {
    ...
    if (typeof value !== 'string' && typeof value !== 'boolean')
      throw new Error('无效的公文参数：' + key)
    if (typeof value === 'string' && value.length > 10000) throw new Error('公文字段过长：' + key)
    args[key] = value
  }
  const format = args.format || 'ordinary'
  if (!['ordinary', 'formal', 'letter', 'command', 'minutes'].includes(format))
    throw new Error('无效的公文版式。')
  if (args.letterhead && !['preprinted', 'digital'].includes(args.letterhead))
    throw new Error('无效的红头模式。')
  if (args['page-number'] && !['center', 'standard', 'none'].includes(args['page-number']))
    throw new Error('无效的页码模式。')
  const warning = reportFontStatus(args)
  const xml = buildDocument(parseMarkdown(markdown), args)
  const bytes = await writeDocx(xml, resolvePageNumberMode(args, format), format)
  return { bytes, warnings: warning ? [warning] : [] }
}
