...
export function packedCircles(roots: HierarchyNode[]) {
  ...
  const root = hierarchy<HierarchyNode>({ id: '', name: '', value: 0, children })
    ...
    .sort(
      (a, b) => b.value! - a.value! || (a.data.id < b.data.id ? -1 : a.data.id > b.data.id ? 1 : 0),
    )
    ...
    series: [
      {
        ...
        renderItem: (params, api) => {
          ...
          const cy = 90 + node.y * size,
            r = node.r * size
          const text = `${node.name}\n${node.value}`
          const bounds = format.getTextRect(
            text,
            '14px sans-serif',
            'center',
            'middle',
            ...
            children: [
              {
                ...
                style: {
                  ...
                  stroke: node.leaf ? '#475569' : '#94a3b8',
                  lineWidth: 1,
                },
