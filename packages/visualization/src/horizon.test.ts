...
it('splits zero and band crossings before folding signed values', () => {
  ...
  const option = buildChartOption({ chartId: 'horizon', x: 'time', y: ['value'],
    table: { columns: ['time', 'value'], rows: [[0, -3], [6, 3]] } })
...
it('shares a band scale across series, preserves gaps and parses dates strictly', () => {
  const data = horizonData(['2026-01-03', '2026-01-01', '2026-01-02'], [[3, -3, null], [9, 0, 0]])
  expect(data.band).toBe(3)
  expect(data.dates).toBe(true)
  expect(data.series[0]!.map((p) => p[1])).toEqual([-3, null, 3])
  expect(horizonData([0, 1], [[0, 0]]).series[0]).toEqual([[0, 0], [1, 0]])
...
it('keeps extreme signed values finite when interpolating', () => {
  ...
  expect(data.series[0]!.every((point) => point.every((v) => v !== null && Number.isFinite(v)))).toBe(true)
})
it('exports a folded chart with explicit sign and band legends', () => {
  ...
  const svg = renderChartSvg({ chartId: 'horizon', title: '盈亏变化', x: '时间', y: ['盈亏'],
    table: { columns: ['时间', '盈亏'], rows: [[0, -3], [1, null], [2, 1], [3, 3]] } })
