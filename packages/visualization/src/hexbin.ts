...
export function hexAddress(x: number, y: number): [number, number] {
  ...
  let a = Math.round(q), b = Math.round(r)
...
export function hexBins(points: readonly Point[]) {
  ...
  const x = extent(0), y = extent(1)
  const sx = (x[1] - x[0]) / 2, sy = y[1] - y[0]
  ...
  const bins = [...cells.values()].sort((a, b) => a.r - b.r || a.q - b.q).map((cell) => {
    const cx = radius * Math.sqrt(3) * (cell.q + cell.r / 2)
    const cy = radius * 1.5 * cell.r
    const vertices = Array.from({ length: 6 }, (_, i) => {
      const angle = (60 * i - 30) * Math.PI / 180
      return [x[0] + (cx + radius * Math.cos(angle)) * sx,
        y[0] + (cy + radius * Math.sin(angle)) * sy] as [number, number]
    })
...
export function hexbinOption(points: readonly Point[], xName: string, yName: string, title = ''): EChartsOption {
  ...
  return {
    ...
    title: { text: title, left: 'center', subtext: `六边形内观测计数 · 共 ${points.length} 条 · 空箱留白` },
    grid: { left: 85, right: 45, top: 85, bottom: 105 },
    xAxis: { type: 'value', name: xName, min: bounds.x[0], max: bounds.x[1], nameLocation: 'middle', nameGap: 28, axisLabel, axisLine: { onZero: false } },
    yAxis: { type: 'value', name: yName, min: bounds.y[0], max: bounds.y[1], nameLocation: 'middle', nameGap: 50, axisLabel, axisLine: { onZero: false } },
    tooltip: { trigger: 'item', renderMode: 'richText' },
    visualMap: { min: 0, max: maximum, dimension: 2,
      orient: 'horizontal', left: 'center', bottom: 12, text: [`${maximum} 条观测`, '0'],
      inRange: { color: ['#eff6ff', '#60a5fa', '#1e40af'] } },
    series: [{ type: 'custom', dimensions: ['箱中心 X', '箱中心 Y', '观测数'],
      encode: { x: 0, y: 1, tooltip: [0, 1, 2] },
      data: bins.map((b) => [...b.center, b.count]),
      renderItem: (params, api) => ({ type: 'polygon',
