...
export function horizonOption(
  ...
  const bandNames = ['正', '负'].flatMap((sign) =>
    [0, 1, 2].map((i) => `${sign} ${format(i * data.band)}–${format((i + 1) * data.band)}`),
  )
  return {
    backgroundColor: '#fff',
    animation: false,
    ...
    xAxis: names.map((_, i) => ({
      type: data.dates ? ('time' as const) : ('value' as const),
      gridIndex: i,
      min: data.series[i]![0]![0],
      max: data.series[i]!.at(-1)![0],
      ...
      formatter: (input) => {
        ...
        for (const item of items) {
          ...
          if (Array.isArray(item.value))
            rows.set(
              row,
              `${names[row]}：${item.value[2] === null ? '缺失' : format(Number(item.value[2]))}`,
            )
        }
        return `${heading}\n${[...rows.values()].join('\n')}`
      },
    },
    series: data.series.flatMap((points, index) =>
      [1, -1].flatMap((sign, signIndex) =>
        [0, 1, 2].map((layer) => ({
          name: bandNames[signIndex * 3 + layer]!,
          type: 'line' as const,
          xAxisIndex: index,
          yAxisIndex: index,
          showSymbol: false,
          ...
          dimensions: ['时间', '折叠高度', '原始值'],
          encode: { x: 0, y: 1, tooltip: [2] },
          data: points.map(([x, y]) => [
            x,
            y === null ? null : Math.max(0, Math.min(data.band, sign * y - layer * data.band)),
            y,
          ]),
        })),
      ),
