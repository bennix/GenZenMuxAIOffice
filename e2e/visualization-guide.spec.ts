...
test('Excel guides data selection, AI chart choice and review of displayed columns', async () => {
  ...
  const zip = await JSZip.loadAsync(await readFile(resolve('apps/sheets/fixtures/generated/compatibility-basic.xlsx')))
  const rows = [['地区','收入','利润','备注'],['东区',120,30,'不发送此列'],['西区',80,10,'内部备注'],['南区',100,20,'仅保留在表格']]
  const xml = rows.map((row,i)=>`<row r="${i+1}">${row.map((value,j)=>{
  ...
  zip.file('xl/worksheets/sheet1.xml',`<?xml version="1.0" encoding="UTF-8"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><dimension ref="A1:D4"/><sheetData>${xml}</sheetData></worksheet>`)
  await writeFile(workbook,await zip.generateAsync({type:'nodebuffer'}))
  const original = await readFile(workbook)
  const launched=await launchShell({openFile:workbook,onboardingSeen:true,recordVideo:false,lang:'zh',videoDir:'visualization-guide'})
  try {
    ...
    await launched.app.evaluate(({ipcMain})=>{
      ...
      ipcMain.handle('ai:chat',(_event,request)=>{
        ...
        if(data.columns.some((column:any)=>column.name==='备注') || data.sample.some((row:any[])=>row.length!==3) || !data.instruction.includes('比较各地区'))
          return {ok:false,error:'字段或目的传递错误'}
        return {ok:true,content:JSON.stringify({chartId:'grouped-column',x:'地区',y:['收入','利润'],title:'地区经营比较',reason:'将收入和利润并列显示，比较不同地区。'})}
      })
    })
    ...
    await expect(guide.getByRole('checkbox',{name:'使用列 备注',exact:true})).not.toBeChecked()
    await guide.getByRole('button',{name:'自己选择图表'}).click()
    await expect(studio.getByLabel('类别 / X / 节点列',{exact:true}).locator('option',{hasText:'备注'})).toHaveCount(0)
    expect(await readFile(workbook)).toEqual(original)
  } finally {await closeAndSaveVideo(launched,'visualization-guide')}
})
