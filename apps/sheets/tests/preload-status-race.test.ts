...
it('does not let a late viewport response overwrite a completed workbook status', async () => {
  ...
  const runtime = { univerAPI: { getActiveWorkbook: () => ({ getActiveSheet: () => worksheet }) } } as unknown as UniverRuntime
  const state = {
    file: { sessionId: 'session', name: 'chart.xlsx', sheets: [{ id: 's', rowCount: 3, columnCount: 2, tables: [] }] },
    flags: { preloadComplete: false }, formulaMode: true, editJournal: createEditJournal(),
    loadedRanges: new Map(), loadingKeys: new Map(), retryTimers: new Map(), sheetProtections: new Map(),
    appliedFilterSheets: new Set(), appliedDvSheets: new Set(), appliedCfSheets: new Set(),
  ...
  finish({ indexingComplete: true, indexedThroughRow: 2, cells: [], rows: [], merges: [], hyperlinks: [], conditionalRules: [], dataValidations: [], autoFilter: null })
  await pending
