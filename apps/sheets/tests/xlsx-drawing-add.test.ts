...
describe('visual additions: shapes and text boxes', () => {
  it('persists editable infographic source on an image description', async () => {
    const plan = await planWith([
      {
        sheetName: 'Data',
        anchor: ANCHOR,
        image: {
          mediaType: 'image/png',
          base64: 'iVBORw0KGgo=',
          description: 'zenoffice-infographic:infographic%20chart-column-simple',
        },
      },
    ])
    expect(plan.added.get('xl/drawings/drawing1.xml')).toContain(
      'descr="zenoffice-infographic:infographic%20chart-column-simple"',
    )
  })
