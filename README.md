...
[最新 Release 下载签名并经 Apple 公证的 DMG](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/GenOffice-0.6.48-arm64.dmg)。
Windows 10/11 x64 用户可下载
[GenOfficeSetup-0.6.48-x64.exe](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/GenOfficeSetup-0.6.48-x64.exe)；当前 Windows 安装包未做 Authenticode 签名，SmartScreen 可能显示“未知发布者”。
Ubuntu 22.04/24.04 x86_64 用户可下载
[genoffice_0.6.48_amd64.deb](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/genoffice_0.6.48_amd64.deb)。
Fedora、RHEL、Rocky Linux、AlmaLinux 与 openSUSE x86_64 用户可下载
[genoffice-0.6.48.x86_64.rpm](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/genoffice-0.6.48.x86_64.rpm)。
...
- **Preview-matched Word PDF export** — paginated preview clones retain the measured content width during printing, so centered and full-width pictures stay at the same horizontal position and physical size in the exported PDF. Editor zoom is reset only for export, and selection chrome is omitted.
- **Stable responsive Word pictures** — declared DOCX picture extents now scale both axes together in print, Web Layout and paginated preview. Pressing a resize handle without dragging no longer rewrites or collapses the picture size, while horizontal dragging resizes proportionally.
...
| ------------------------------------------------------- | --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| **macOS** — Apple Silicon (arm64)                       | macOS 11+                                     | [GenOffice-0.6.48-arm64.dmg](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/GenOffice-0.6.48-arm64.dmg)       |
...
| **Windows** (x64)                                       | Windows 10/11; unsigned, SmartScreen may warn | [GenOfficeSetup-0.6.48-x64.exe](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/GenOfficeSetup-0.6.48-x64.exe) |
| **Ubuntu** — DEB (amd64)                                | Ubuntu 22.04/24.04 x86_64                     | [genoffice_0.6.48_amd64.deb](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/genoffice_0.6.48_amd64.deb)       |
| **Linux RPM** — Fedora / RHEL / Rocky / Alma / openSUSE | x86_64, glibc 2.34+ (RHEL-compatible 9+)      | [genoffice-0.6.48.x86_64.rpm](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/genoffice-0.6.48.x86_64.rpm)     |
...
```bash
sudo apt install ./genoffice_0.6.48_amd64.deb
```

On Fedora / RHEL-family / openSUSE, install the rpm instead:

```bash
sudo dnf install ./genoffice-0.6.48.x86_64.rpm     # Fedora / RHEL / Rocky / Alma
sudo zypper install ./genoffice-0.6.48.x86_64.rpm  # openSUSE
```
