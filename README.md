...
[最新 Release 下载 Developer ID 签名并已完成 Apple 公证的 DMG](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/GenOffice-0.6.63-arm64.dmg)。
Windows 10/11 x64 用户可下载
[GenOfficeSetup-0.6.63-x64.exe](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/GenOfficeSetup-0.6.63-x64.exe)；当前 Windows 安装包未做 Authenticode 签名，SmartScreen 可能显示“未知发布者”。
Ubuntu 22.04/24.04 x86_64 用户可下载
[genoffice_0.6.63_amd64.deb](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/genoffice_0.6.63_amd64.deb)。
Fedora、RHEL、Rocky Linux、AlmaLinux 与 openSUSE x86_64 用户可下载
[genoffice-0.6.63.x86_64.rpm](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/genoffice-0.6.63.x86_64.rpm)。
...
- **WeChat three-day Markdown diary** — bind WeChat 8.0.70+ through its official ClawBot flow, then send text to GenOffice while the desktop app is running. Messages and optional replies from the currently configured ZenMux/OpenAI-compatible AI are appended to a local Markdown diary grouped into three-day windows with a 4:00 a.m. rollover. `记：…`, `撤回`, `结束`, and `帮助` provide save-only, undo, seal, and help commands; binding credentials are OS-encrypted and network calls are restricted to official WeChat HTTPS hosts.
...
| ------------------------------------------------------- | --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| **macOS** — Apple Silicon (arm64)                       | macOS 11+                                     | [GenOffice-0.6.63-arm64.dmg](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/GenOffice-0.6.63-arm64.dmg)       |
...
| **Windows** (x64)                                       | Windows 10/11; unsigned, SmartScreen may warn | [GenOfficeSetup-0.6.63-x64.exe](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/GenOfficeSetup-0.6.63-x64.exe) |
| **Ubuntu** — DEB (amd64)                                | Ubuntu 22.04/24.04 x86_64                     | [genoffice_0.6.63_amd64.deb](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/genoffice_0.6.63_amd64.deb)       |
| **Linux RPM** — Fedora / RHEL / Rocky / Alma / openSUSE | x86_64, glibc 2.34+ (RHEL-compatible 9+)      | [genoffice-0.6.63.x86_64.rpm](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/genoffice-0.6.63.x86_64.rpm)     |
...
```bash
sudo apt install ./genoffice_0.6.63_amd64.deb
```

On Fedora / RHEL-family / openSUSE, install the rpm instead:

```bash
sudo dnf install ./genoffice-0.6.63.x86_64.rpm     # Fedora / RHEL / Rocky / Alma
sudo zypper install ./genoffice-0.6.63.x86_64.rpm  # openSUSE
```
