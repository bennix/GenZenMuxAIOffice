...
[最新 Release 下载签名并经 Apple 公证的 DMG](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/GenOffice-0.6.42-arm64.dmg)。
Windows 10/11 x64 用户可下载
[GenOfficeSetup-0.6.42-x64.exe](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/GenOfficeSetup-0.6.42-x64.exe)；当前 Windows 安装包未做 Authenticode 签名，SmartScreen 可能显示“未知发布者”。
Ubuntu 22.04/24.04 x86_64 用户可下载
[genoffice_0.6.42_amd64.deb](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/genoffice_0.6.42_amd64.deb)。
Fedora、RHEL、Rocky Linux、AlmaLinux 与 openSUSE x86_64 用户可下载
[genoffice-0.6.42.x86_64.rpm](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/genoffice-0.6.42.x86_64.rpm)。
...
- **Restored PDF workspace** — open PDFs from the dedicated AI PDF home card, the PDF-only picker, Finder, or Explorer; scroll or page through, zoom, fit and hand-pan; edit text while preserving the original font family, size, weight, style and mixed-style boundaries; extract or insert pages, freely place/move/resize floating images above the page and text or send them below page text, add watermarks, open password-protected files, and save an AES-128 password-protected copy without modifying the source.
- **ZenMux PDF equation recognition** — paste a formula screenshot or choose an image, recognize it through the ZenMux vision model as editable LaTeX, correct multi-line source with live mathematical preview, then place a transparent high-resolution equation object back onto the PDF at the intended size and position.
- **Reliable scientific PDF text and table editing** — pdfTeX discretionary hyphens are normalized across the PDF renderer and editor, so paragraphs containing inline equations remain editable; narrow table columns open as individual cells instead of unsafe whole-column replacements. Text rebuilds prefer the embedded face, then the exact installed face, then a glyph-compatible serif/sans/monospace fallback; the live editor retains the source ink and samples colored page or table-cell backdrops instead of masking them white. Image-edit mode explains when a figure is vector text/path content rather than an independently editable bitmap.
...
- **`@Connect` cross-editor flow** — send the latest AI reply to any other open Word, Excel, PowerPoint, or Markdown tab through a local-only channel; the destination receives editable rich text, cells, text boxes, or Markdown rather than a flattened image.
...
| ------------------------------------------------------- | --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| **macOS** — Apple Silicon (arm64)                       | macOS 11+                                     | [GenOffice-0.6.42-arm64.dmg](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/GenOffice-0.6.42-arm64.dmg)       |
...
| **Windows** (x64)                                       | Windows 10/11; unsigned, SmartScreen may warn | [GenOfficeSetup-0.6.42-x64.exe](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/GenOfficeSetup-0.6.42-x64.exe) |
| **Ubuntu** — DEB (amd64)                                | Ubuntu 22.04/24.04 x86_64                     | [genoffice_0.6.42_amd64.deb](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/genoffice_0.6.42_amd64.deb)       |
| **Linux RPM** — Fedora / RHEL / Rocky / Alma / openSUSE | x86_64, glibc 2.34+ (RHEL-compatible 9+)      | [genoffice-0.6.42.x86_64.rpm](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/genoffice-0.6.42.x86_64.rpm)     |
...
```bash
sudo apt install ./genoffice_0.6.42_amd64.deb
```

On Fedora / RHEL-family / openSUSE, install the rpm instead:

```bash
sudo dnf install ./genoffice-0.6.42.x86_64.rpm     # Fedora / RHEL / Rocky / Alma
sudo zypper install ./genoffice-0.6.42.x86_64.rpm  # openSUSE
```
