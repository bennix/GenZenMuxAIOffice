...
ZenOffice is a free, open-source alternative to Microsoft Office for macOS,
...
[最新 Release 下载 Developer ID 签名并已完成 Apple 公证的 DMG](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/ZenOffice-0.6.83-arm64.dmg)。
Windows 10/11 x64 用户可下载
[ZenOfficeSetup-0.6.83-x64.exe](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/ZenOfficeSetup-0.6.83-x64.exe)；当前 Windows 安装包未做 Authenticode 签名，SmartScreen 可能显示“未知发布者”。
Ubuntu 22.04/24.04 x86_64 用户可下载
[zenoffice_0.6.83_amd64.deb](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/zenoffice_0.6.83_amd64.deb)。
Fedora、RHEL、Rocky Linux、AlmaLinux 与 openSUSE x86_64 用户可下载
[zenoffice-0.6.83.x86_64.rpm](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/zenoffice-0.6.83.x86_64.rpm)。
...
- **Persistent Word picture edits** — picture replacement and AI image cleanup now overwrite the original DOCX media relationship in place, remove stale image relationships, and preserve the original display size, so Word, WPS, and LibreOffice reopen the same edited picture shown in ZenOffice.
...
- **PDF editing with portable font fallback** — open PDFs from the dedicated AI PDF home card, Finder, or Explorer; edit selected text through the ribbon while preserving the original run whenever possible. When the source font is unavailable or subset-only, ZenOffice resolves a local equivalent or embeds a bundled Noto/Liberation subset for Latin, Simplified/Traditional Chinese, Japanese, and Korean text. Page operations, images, annotations, signatures, stamps, password opening, and AES-128 protected copies remain available.
...
- **Resilient third-party XLSX display** — when an exported workbook contains real rows and columns but incorrectly declares its used range as only `A1`, ZenOffice verifies the sheet and recovers the actual extent instead of showing only the title or a blank grid. Existing values, Chinese fonts, borders, row heights, and column widths remain untouched.
- **Selectable AI slide beautification** — AI Beautify now opens three visual design proposals in the side panel before changing the slide. Each proposal previews its font pairing and palette, with an optional large-type presentation mode; regenerated slides enforce readable title and body-size floors and retry outputs that fail the typography audit.
- **Reliable AI slide layers** — regenerated slides preserve explicit CSS stacking and automatically move late-emitted background panels behind the text, images, labels and cards they contain, preventing large decorative objects from covering smaller editable content.
- **Office/WPS-compatible PPTX saves** — presentations saved by ZenOffice now use broadly compatible ZIP headers and repair missing OOXML namespace declarations, preventing generated or AI-beautified decks from opening as blank pages in other office suites.
- **Reliable PowerPoint print preview and PDF export** — image-heavy or long presentations are loaded from a temporary local HTML file instead of being embedded in an oversized Chromium data URL, preventing `ERR_INVALID_URL` while preserving every slide through native printing and PDF export.
...
- **Open-file `@mentions` in AI prompts** — type `@` in the Word, Excel, PowerPoint, Markdown, or PDF AI composer to choose another saved file already open in ZenOffice. The selected file is attached through the same local parsing and five-attachment pipeline, while the menu, empty state, and errors follow all 19 application languages.
- **PowerPoint typography lock for AI edits** — slide regeneration and editable HTML write-back preserve the deck's designed title/body scale instead of baking an autofit-reduced display size back into PPTX. Explicit font-size edits disable stale shrink-to-fit metadata so the requested size remains visible and editable.
...
- **Source-preserving infographics across editors** — Word, Excel, PowerPoint, and Markdown share the AntV infographic studio. DOCX, XLSX, and PPTX save both the rendered PNG and editable source metadata, so a reopened infographic can be double-clicked, revised with ZenMux AI, and replaced in place without changing its frame or position. Markdown source editing returns to the live preview when focus leaves the block.
...
- **Recoverable WeChat text-and-image diary** — bind WeChat 8.0.70+ through its official ClawBot flow, then send text or images while ZenOffice is running. Images are decrypted from WeChat's official CDN into a local companion asset folder and embedded by relative path in the three-day Markdown diary. Image-only messages stay silent until five images are collected; a fifth image, or a question sent after any smaller batch, invokes the current ZenMux multimodal model. A durable outbox verifies iLink business success and writes the AI block only after WeChat confirms delivery; stable client IDs retry interrupted or chunked long replies without duplicating diary blocks or repeating the AI call. `记：…`, `撤回`, `结束`, and `帮助` remain available; credentials are OS-encrypted and token-bearing requests stay on official WeChat HTTPS hosts.
- **WeChat PDF multi-round review** — send one encrypted PDF attachment from WeChat and ZenOffice saves it beside the three-day Markdown diary, asks for the same Science/Nature/Cell/Elsevier/IEEE/grant/bid profile exposed by the desktop review UI, and asks whether the report should be Chinese or English. After selection, ZenMux performs chunked evidence extraction, three independent reviewer calls, and a separate chair synthesis. Every completed stage is persisted for network-resumable review; the long final report is delivered to WeChat in stable chunks and written back to Markdown. Downloads are restricted to the official WeChat CDN, limited to 50 MB, decrypted locally, and validated by the PDF signature.
...
| ------------------------------------------------------- | --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| **macOS** — Apple Silicon (arm64)                       | macOS 11+                                     | [ZenOffice-0.6.83-arm64.dmg](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/ZenOffice-0.6.83-arm64.dmg)       |
...
| **Windows** (x64)                                       | Windows 10/11; unsigned, SmartScreen may warn | [ZenOfficeSetup-0.6.83-x64.exe](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/ZenOfficeSetup-0.6.83-x64.exe) |
| **Ubuntu** — DEB (amd64)                                | Ubuntu 22.04/24.04 x86_64                     | [zenoffice_0.6.83_amd64.deb](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/zenoffice_0.6.83_amd64.deb)       |
| **Linux RPM** — Fedora / RHEL / Rocky / Alma / openSUSE | x86_64, glibc 2.34+ (RHEL-compatible 9+)      | [zenoffice-0.6.83.x86_64.rpm](https://github.com/bennix/GenZenMuxAIOffice/releases/latest/download/zenoffice-0.6.83.x86_64.rpm)     |
...
The deb installs with apt — it pulls in the dependencies and adds ZenOffice
to the applications menu:

```bash
sudo apt install ./zenoffice_0.6.83_amd64.deb
```

On Fedora / RHEL-family / openSUSE, install the rpm instead:

```bash
sudo dnf install ./zenoffice-0.6.83.x86_64.rpm     # Fedora / RHEL / Rocky / Alma
sudo zypper install ./zenoffice-0.6.83.x86_64.rpm  # openSUSE
```
...
| --------------- | ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apps/docs`     | **ZenOffice Docs**     | `.docx` word processor. Byte-preserving round trip: only dirty paragraphs are regenerated (paragraph patch), everything else in the original file is kept byte-for-byte, so opening and saving never breaks layout in Word. Paginated view whose line metrics reproduce the original document's layout, tracked changes, comments, styles, equations, ink.                                                                                                                                                                                                      |
| `apps/sheets`   | **ZenOffice Sheets**   | `.xlsx` spreadsheet. UI built on the open-source [Univer](https://github.com/dream-num/univer) core (Apache-2.0) with a large layer of in-house extensions; `.xlsx` import/export runs through an in-house Rust sidecar (calamine + IronCalc), charts are rendered in-house (Konva), plus pivot tables, slicers, conditional formatting, and formula tracing.                                                                                                                                                                                                   |
| `apps/slides`   | **ZenOffice Slides**   | `.pptx` presentations. In-house `.pptx` parse/render/edit engine with masters, charts, cropping, ink, and text shaping (HarfBuzz metrics).                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `apps/pdf`      | **ZenOffice PDF**      | `.pdf` viewer/editor on [pdf.js](https://github.com/mozilla/pdf.js) (Apache-2.0) + [pdf-lib](https://github.com/Hopding/pdf-lib) (MIT): annotations, forms, outlines, stamps, signatures, page operations, and printing support. True text editing — paragraph selection with in-block reflow, alignment restoration, original-font preservation — and content-stream image insert/edit, all rewriting page content streams through [PDFium](https://pdfium.googlesource.com/pdfium/) wasm (BSD-3-Clause) with subset-embedded fonts — no cover-up annotations. |
| `apps/markdown` | **ZenOffice Markdown** | `.md` / `.markdown` editor: Tiptap block editor over plain Markdown files — headings, lists, tables, images, code blocks — saved back as plain Markdown, hosted in shell tabs.                                                                                                                                                                                                                                                                                                                                                                                  |
| `apps/shell`    | **ZenOffice**          | The suite shell: home screen, tabbed hosting of the five editors, light/dark/system theme, auto-update.                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
**Is ZenOffice free?**
Yes. ZenOffice is free and open-source under the Apache-2.0 license — no
trial, no paid tier for the apps themselves.

**Can ZenOffice open Microsoft Word, Excel, and PowerPoint files?**
Yes. ZenOffice opens and saves native `.docx`, `.xlsx`, and `.pptx` files.
...
ZenOffice would not be possible without these open-source projects:
...
ZenOffice is licensed under the [Apache License 2.0](LICENSE), with one
exception: the upstream `ee/` directory retains the separate terms stated in
[its license](ee/LICENSE).

The upstream GenOffice and Genspark names and logos are trademarks of
Mainfunc, Inc. ZenOffice uses its own name and visual identity for this
