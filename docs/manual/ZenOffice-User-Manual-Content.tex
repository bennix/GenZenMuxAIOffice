...
% Representative UI screenshots were captured from an earlier release; this guide is updated for v0.6.88.
...
\ifManualChineseOnly
  ...
  \newcommand{\ManualPdfSubject}{ZenOffice v0.6.88 中文用户指南}
\else\ifManualEnglishOnly
  ...
  \newcommand{\ManualPdfSubject}{ZenOffice v0.6.88 English user manual}
\else
  \newcommand{\CNText}[1]{#1}
  \newcommand{\ENText}[1]{#1}
  ...
  \newcommand{\ManualPdfSubject}{ZenOffice v0.6.88 bilingual getting-started manual}
\fi\fi
...
\fancyhead[R]{\footnotesize\color{GOMuted} v0.6.88 · 2026-09-17}
...
{\color{white!70}\small v0.6.88 · 2026-09-17 · macOS / Windows / Ubuntu / RPM\par}
\clearpage
...
\CNText{本手册面向第一次使用 ZenOffice 的用户，以当前仓库 \textbf{v0.6.86} 源码与真实运行态为依据，说明安装、ZenMux 设置、Word、Excel、PowerPoint、Markdown、PDF、工作簿 SQL 数据库、跨平台打印、跨编辑器 AI 工作流，以及 2026-09-16 新增的 MCP 应用服务补充。界面截图于 2026-08-16 至 17 日在隔离的临时用户目录中从应用运行态取得；截图没有包含用户文件、真实 API Key 或个人数据。演示科研稿与工作簿均为专门制作的合成素材，不代表真实科研或企业结论。}

\ENText{This manual is based on the current \textbf{v0.6.86} source tree and real runtime UI. It covers installation, ZenMux configuration, Word, Excel, PowerPoint, Markdown, PDF, workbook SQL databases, cross-platform printing, cross-editor AI workflows, and the MCP application-services supplement added on 16 September 2026. Screenshots were captured on 16--17 August 2026 in an isolated temporary profile and contain no user documents, real API Key, or personal data. The research draft and workbooks are purpose-built synthetic demonstrations, not real scientific or business findings.}
...
\begin{enumerate}
  ...
  \item Ubuntu：执行 \texttt{sudo apt install ./zenoffice\_0.6.86\_amd64.deb}。Fedora/RHEL 系使用 \texttt{sudo dnf install ./zenoffice-0.6.86.x86\_64.rpm}。
\end{enumerate}
\begin{center}
...
\begin{enumerate}
  ...
  \item Ubuntu: run \texttt{sudo apt install ./zenoffice\_0.6.86\_amd64.deb}. Fedora/RHEL-family users can run \texttt{sudo dnf install ./zenoffice-0.6.86.x86\_64.rpm}.
\end{enumerate}
\begin{center}
\begin{tabularx}{.95\linewidth}{>{\bfseries}p{28mm}p{43mm}X}
\toprule
...
\SmallShot{04-settings-about.png}{设置 > 关于的真实运行态；截图采集于旧版界面，发行与手册版本以页眉标注的 v0.6.86 为准。}
\begin{itemize}
...
\SmallShot{04-settings-about.png}{Real Settings > About runtime; the capture predates this release, so use the v0.6.86 header as the authoritative manual version.}
\begin{itemize}
...
\begin{itemize}
  ...
  \item 打开旧版 .doc 时，应用优先调用本机 LibreOffice 转换为独立 DOCX，以保留字体、字号、颜色、表格与分页。若尚未安装，可从提示框启动对应平台的安装流程：macOS 使用 Homebrew、Windows 使用 winget，Ubuntu/Debian、Fedora/RHEL 与 Arch Linux 使用各自包管理器和系统授权。若无法自动安装则转到官方下载页。应用不会静默降级；只有用户明确确认后才执行纯文字恢复。转换结果另存为新文件，绝不覆盖原始 .doc。
...
\begin{itemize}
  ...
  \item For legacy .doc, ZenOffice first asks local LibreOffice for a high-fidelity conversion to a separate DOCX, preserving fonts, sizes, colors, tables, and pagination. If LibreOffice is missing, the prompt can start the platform-appropriate installer: Homebrew on macOS, winget on Windows, and the native package manager with system authorization on Ubuntu/Debian, Fedora/RHEL, or Arch Linux. Unsupported environments fall back to the official download page. ZenOffice never degrades silently: text-only recovery runs only after explicit user confirmation. The original .doc is never overwritten.
...
\begin{itemize}
  ...
  \item PDF 功能区支持插入 PNG/JPEG/SVG 图片并在页面上放置、移动和缩放；SVG 会作为图片对象写入，不保留可编辑矢量路径。需要大范围文字、公式或排版修改时，仍应优先回到源文件再重新导出 PDF。
\end{itemize}
...
\begin{itemize}
  ...
  \item The PDF ribbon can insert PNG/JPEG/SVG images and place, move, and resize them on the page. SVG is written as an image object rather than retained as editable vector paths. For broad text, equation, or layout changes, edit the source document and export PDF again.
\end{itemize}
...
\begin{enumerate}
  ...
  \item 还可添加文件、拖放或粘贴图片，总数最多 5 个。普通附件在本机解析，图片作为视觉输入。Excel 可视化或 MCP 生成的 SVG 也可插入当前 PDF 页，再由用户保存。
...
\begin{enumerate}
  ...
  \item Add files by picker/drop or paste images, up to five total. Ordinary files are parsed locally; images are visual input. SVG produced by Excel visualization or MCP can also be inserted into the current PDF page, then saved by the user.
...
\begin{enumerate}
  \item 保存后关闭当前文件，重新打开一次；确认图片修改、PDF 插图、公式、表格、引用和水印不是仅存在于临时预览层。
...
\begin{enumerate}
  \item Save, close, and reopen. Confirm that image edits, PDF inserted images, equations, tables, citations, and watermarks are not merely temporary preview layers.
...
% -----------------------------------------------------------------------------
% MCP release supplement (keeps the published base-edition version).
\input{ZenOffice-MCP-Supplement.tex}
...
\vfill
{\color{white}\bfseries ZenOffice v0.6.88 · ZenMux AI\par}
