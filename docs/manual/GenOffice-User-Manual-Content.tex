...
\ifManualChineseOnly
  ...
  \newcommand{\ManualPdfSubject}{ZenOffice v0.6.55 中文用户指南}
\else\ifManualEnglishOnly
  ...
  \newcommand{\ManualPdfSubject}{ZenOffice v0.6.55 English user manual}
\else
  \newcommand{\CNText}[1]{#1}
  \newcommand{\ENText}[1]{#1}
  ...
  \newcommand{\ManualPdfSubject}{ZenOffice v0.6.55 bilingual getting-started manual}
\fi\fi
...
\CNText{本手册面向第一次使用 ZenOffice 的用户，以当前仓库 \textbf{v0.6.55} 源码与真实运行态为依据，说明安装、ZenMux 设置、Word、Excel、PowerPoint、Markdown、PDF、工作簿 SQL 数据库以及跨编辑器 AI 工作流。界面截图于 2026-08-16 至 17 日在隔离的临时用户目录中从当前应用运行态取得；截图没有包含用户文件、真实 API Key 或个人数据。演示科研稿与工作簿均为专门制作的合成素材，不代表真实科研或企业结论。}
...
\vfill
{\color{white}\bfseries ZenOffice v0.6.55 · ZenMux AI\par}
