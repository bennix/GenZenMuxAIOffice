...
% UI screenshots in this manual were captured from the current v0.6.55 source tree.
...
\ifManualChineseOnly
  ...
  \newcommand{\ManualPdfSubject}{GenOffice v0.6.55 中文用户指南}
\else\ifManualEnglishOnly
  ...
  \newcommand{\ManualPdfSubject}{GenOffice v0.6.55 English user manual}
\else
  \newcommand{\CNText}[1]{#1}
  \newcommand{\ENText}[1]{#1}
  ...
  \newcommand{\ManualPdfSubject}{GenOffice v0.6.55 bilingual getting-started manual}
\fi\fi
...
\fancyhead[R]{\footnotesize\color{GOMuted} v0.6.55 · 2026-08-17}
...
{\color{white!70}\small v0.6.55 · 2026-08-17 · macOS / Windows / Ubuntu / RPM\par}
\clearpage
...
\CNText{本手册面向第一次使用 GenOffice 的用户，以当前仓库 \textbf{v0.6.55} 源码与真实运行态为依据，说明安装、ZenMux 设置、Word、Excel、PowerPoint、Markdown、PDF、工作簿 SQL 数据库以及跨编辑器 AI 工作流。界面截图于 2026-08-16 至 17 日在隔离的临时用户目录中从当前应用运行态取得；截图没有包含用户文件、真实 API Key 或个人数据。演示科研稿与工作簿均为专门制作的合成素材，不代表真实科研或企业结论。}

\ENText{This manual is based on the current \textbf{v0.6.55} source tree and real runtime UI. It covers installation, ZenMux configuration, Word, Excel, PowerPoint, Markdown, PDF, workbook SQL databases, and cross-editor AI workflows. Screenshots were captured on 16--17 August 2026 in an isolated temporary profile and contain no user documents, real API Key, or personal data. The research draft and workbooks are purpose-built synthetic demonstrations, not real scientific or business findings.}
...
\begin{enumerate}
  ...
  \item Ubuntu：执行 \texttt{sudo apt install ./genoffice\_0.6.55\_amd64.deb}。Fedora/RHEL 系使用 \texttt{sudo dnf install ./genoffice-0.6.55.x86\_64.rpm}。
\end{enumerate}
\begin{center}
...
\begin{enumerate}
  ...
  \item Ubuntu: run \texttt{sudo apt install ./genoffice\_0.6.55\_amd64.deb}. Fedora/RHEL-family users can run \texttt{sudo dnf install ./genoffice-0.6.55.x86\_64.rpm}.
\end{enumerate}
\begin{center}
\begin{tabularx}{.95\linewidth}{>{\bfseries}p{28mm}p{43mm}X}
\toprule
...
\SmallShot{04-settings-about.png}{设置 > 关于的真实运行态；截图采集于旧版界面，发行与手册版本以页眉标注的 v0.6.55 为准。}
\begin{itemize}
...
\SmallShot{04-settings-about.png}{Real Settings > About runtime; the capture predates this release, so use the v0.6.55 header as the authoritative manual version.}
\begin{itemize}
...
\vfill
{\color{white}\bfseries GenOffice v0.6.55 · ZenMux AI\par}
